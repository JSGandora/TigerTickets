class AccountController < ApplicationController
  ...
  def mytix
    ...
    buyRequests = BuyRequest.select("show_id, title, time, location, \"group\", img, buy_requests.id AS buy_request_id, status").where(netid: netid).where(:status => ["waiting-for-match", "pending", "completed"]).joins("INNER JOIN shows ON shows.id = buy_requests.show_id")
    sellRequests = SellRequest.select("show_id, title, time, location, \"group\", img, sell_requests.id AS sell_request_id, status").where(netid: netid).where(:status => ["waiting-for-match", "pending", "completed"]).joins("INNER JOIN shows ON shows.id = sell_requests.show_id")
